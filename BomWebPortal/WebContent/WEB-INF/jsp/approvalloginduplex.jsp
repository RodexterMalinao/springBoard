...
<form:form method="POST" action="approvalloginduplex.html" name="loginForm" commandName="approvalLoginDuplexForm">
<div class="supportDocForm">
	<h3 class="table_title">Approval Authen</h3>
	...
	<div class="buttonmenubox_R" id="buttonArea">
		...
		<input type="hidden" name="currentView" value="approvalloginduplex.html">
		<input type="hidden" id="approvalInd" value="${approvalLoginDuplexForm.approvalInd }"/>
		<form:errors path="approvalInd" cssClass="error"/>
		<a href="#" class="nextbutton login_button">Confirm</a>
	</div>
</div>
</form:form>
  <script>
  if($("#approvalInd").val() == 'true'){
	  window.close();
  }
