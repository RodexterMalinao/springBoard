...
<script type="text/javascript" src="js/web/lts/ltsdnchange.js?v=<fmt:formatDate pattern="yyyyMMddHH24mmss" value="<%=new java.util.Date()%>" />"></script>
...
<form:form method="POST" id="ltsDnChangeForm" name="ltsDnChangeForm" commandName="ltsdnchangeCmd" autocomplete="off">
...
<table width="100%" class="paper_w2 round_10">
	<tr>
		...
		<c:if test="${ltsdnchangeCmd.duplexProfile}">
			...
			<table width="100%" border="0" cellspacing="4" cellpadding="6" class="contenttext">				
				<tr>
				    <td width="30%"></td>
					<td width="20%"><div align="left"><b> DNX (${fn:substring(sessionLtsOrderCapture.ltsServiceProfile.srvNum,4,12)})</b>
					<input type="hidden" value="${ltsdnchangeCmd.duplexProfile}" id="duplexProfileInd" />
					</div></td>
		...
		<table border="0" cellspacing="4" cellpadding="6" class="contenttext">			
			...
			<c:if test="${not empty ltsdnchangeCmd.numSelectionList}">
			<c:forEach items="${ltsdnchangeCmd.numSelectionList}" var="itemList" varStatus="itemListStatus">
			<div align="left">
		...
		<table border="0" cellspacing="4" cellpadding="6" class="contenttext">			
			...
			<c:if test="${not empty ltsdnchangeCmd.reservedDnList}">
			<c:forEach items="${ltsdnchangeCmd.reservedDnList}" var="itemList" varStatus="itemListStatus">
			<div align="left">
			<table border="0" cellspacing="1" cellpadding="1" class="contenttext">	
		    <tr><td>
		...
		<c:if test="${not empty ltsdnchangeCmd.chargeItem}">
				<tr id="chargeItems">
					<td colspan="5">
						<table width="100%">
							<tr>
											...
											<td>
												<table class="table_style" width="100%" border="1">
													<tr>
														...
														<tr class="" >														
															<td align="center"><c:out value="${ltsdnchangeCmd.chargeItem.itemDisplayHtml}" default="-" /></td>
															<td align="center"><c:out value="${ltsdnchangeCmd.chargeItem.oneTimeAmtTxt}" default="-"/></td>
															<td class="penaltyHandling" align="center">
															        <input type="hidden" id="channel" value="${ltsdnchangeCmd.channelID}" />
																	...
																	<div id="approveBtn" class="func_button" align="right">                       
                                                                       <a type="hidden" id="approvalBtn" href="approvallogin.html" onclick="window.open(this.href, 'mywin','left=20,top=20,width=500,height=500,toolbar=1,resizable=0'); return false;" >Approval</a>                                                                       
                                                                    </div>
                                                                    <input type="hidden" id="penaltyApprovalInd" value="${ltsdnchangeCmd.penaltyApprovalInd}" />
															</td>
														</tr>
												      <c:if test="${not empty ltsdnchangeCmd.rebateItem}">
														<tr id="rebateItem" style="display: none;">														
														<td align="center"><c:out value="${ltsdnchangeCmd.rebateItem.itemDisplayHtml}" default="-" /></td>
														<td align="center"><c:out value="${ltsdnchangeCmd.rebateItem.oneTimeAmtTxt}" default="-"/></td>
