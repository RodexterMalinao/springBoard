...
<script language="javascript">
	...
	$(document).ready(function() {
		...
		$("#searchButton").click(function(e) {
			...
			var $tbody = $(".selectResultTableContext tbody");
			$tbody.empty();
			$tbody.append($("<tr/>").append($("<td/>")).append($("<td/>").attr("colspan", 2).text("Loading...")));
			$.ajax({
				url: "mobccsstockmanualsearch.html"
				, data: {
				...
				, success: function(data) {
					if (!data || data.length == 0) {
						...
						$.each(data, function(index, item) {
												...
												function(i2, item2) {
													if (item2.result == "N") {
														...
														$.ajax({
															...
															, dataType: "json"
															, success: function(data) {
																window.open('mobccsstockmanualdetail.html?'+ $.param({"orderId": orderId}), '_blank', 'toolbar=0,location=0,menubar=0,height=768,width=1024');
															}
															, error: function() {
							...
							var $td1 = $("<td/>").attr("align", "center").text(item.deliveryDate);
							var $td2 = $("<td/>").attr("align", "center").text(item.requestDate);
							var $td3 = $("<td/>").attr("align", "center").text(item.salesId);
							var $td4 = $("<td/>").attr("align", "center").text(item.orderStatus);
							
							$tbody.append($tr.append($td0).append($td1).append($td2).append($td3).append($td4));
						});
...
<form:form name="stockManualForm" method="POST" commandName="stockManual">
	...
	<div style="text-align: center; padding: 5px 0" class="selectResultTable">
		<span class="control">
			...
			<%-- ********************************   BOTTOM BUTTONS   ************************************* --%>
			<div>
				<table class="smalltextgray">
				<tr>
				<td style="display: inline-block; float:left;">
					* Order ID search apply to below order status:
				</td>
				</tr>
				<tr>
				<td style="display: inline-block; float:left">
					Pending - Stock Assignment (Success)
				</td>
				</tr>
				<tr>
				<td style="display: inline-block; float:left">
					Pending - Support Document (Success)
				</td>
				</tr>
				<tr>
				<td style="display: inline-block; float:left">
					Pending - Payment Settlement (Success)
				</td>
				</tr>
				<tr>
				<td style="display: inline-block; float:left">
					F&S Follow up - Urgent order processing
				</td>
				</tr>
				<tr>
				<td style="display: inline-block; float:left">
					Fallout - Stock Assignment (Out of Stock)
				</td>
				</tr>
				<tr>
				<td style="display: inline-block; float:left">
					F&S Follow up - Stock Assignment (Manual Assign)
				</td>
				</tr>			
				</table>

			</div>
			<a href="mobccsstock.html" class="nextbutton" style="display: inline-block; float:right">
				Quit
			</a>
	</div>
