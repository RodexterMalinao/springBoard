...
<script language="javascript">
	...
	function onSearch(){	
		...
		$.ajax({
			url: "mobdsstockmanualassgnsearch.html"
			, data: {
				  itemCode: $("#ic").val()
				, orderId: $("#oi").val()
			}
	...
	function onAssign(){
		...
		if (document.stockManualAssgnForm.selItemSerial[0] == null) {
	...
	function onQuit(){
		...
		if ($("#sourceId").val() == "DOA") {
			...
			link = "mobdsstockmanualdetail.html?orderId=" + $("#oi").val();
		}
...
<form:form name="stockManualAssgnForm" method="POST" commandName="stockManualAssgn">
			...
			<td bgcolor="#FFFFFF">
				<table width="100%" border="0" cellspacing="1" rules="none">
					<tr>
						...
						<c:choose>
						<c:when test="${hasOtherColor}">
							<td>Item Code:</td>
							<td>
								<form:select path="itemCode" id="ic" >
									<form:options items="${itemCodeList}" itemValue="codeId" itemLabel="codeDesc"/>
								</form:select>
							</td>
						</c:when>
						<c:otherwise>
	...
	<div style="text-align: center; padding: 5px 0" class="selectResultTable">
		...
		<input type="button" id="quitButton" value="Quit" onClick="onQuit()" style="display: inline-block; float:right">
		<c:choose>
				<c:when test="${action == 'DOA'}">
					<input type="button" id="assignButton" value="DOA Assign" onClick="onAssign()" style="display: inline-block; float:right">
				</c:when>
				<c:otherwise>
