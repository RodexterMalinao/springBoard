...
<script language="javascript">
	...
	$(document).ready(function() {
		...
		/* $('#resulttable input[type=button]').click(function() { */
			var itemCode;
			var itemSerial;
			itemCode = $(this).closest('tr').find("td:eq(3)").text();
			itemSerial = $(this).closest('tr').find("td:eq(5)").text();
			...
			<td>
				...
				<table width="100%" border="1" cellspacing="0" class="contenttext" bgcolor="#FFFFFF" id="resulttable">
					<tr>
						...
						<th class="blueHeader">Previous Item Code</th>
					...
					<c:forEach items="${stockManualDetailList}" var="result">
						<tr>
							...
							<c:choose>
								<c:when test='${result.serialNum == "" || empty result.serialNum}'>
									<td>
										<input type="button" id="doaButton" value="DOA" disabled="disabled">
									</td>
								</c:when>
								<c:otherwise>
									<td>
										<input type="button" id="doaButton" value="DOA" onClick="window.location='mobdsstockmanualassgn.html?orderId=${orderId}&itemCode=${result.itemCode}&action=DOA' ">
									</td>
								</c:otherwise>
							</c:choose>
							...
							<td>
								<a href="mobdsstockmanualassgn.html?itemCode=${result.itemCode}&orderId=${orderId}&action=ASSIGN">
									&nbsp;${result.itemCode}&nbsp;
								</a>
							</td>
