...
<script>
	...
	function openDialog4SumNTV(orderId){	
		   openModalDialog('${ntvDomain}'+"ntvorderdetail.html?orderId=" + orderId + "&_al=true", "Order Id :"+orderId);	
	}
...
// 			$('#dateDatepickerEnd').datepicker( "option", "maxDate", new Date (2999,1,1));
		...
		function submitForm() {
			if(isInputOkayBeforeSubmit()){ 
				document.dsQCImsOrderEnquiryForm.submit();
			}
		}
...
<form:form name="dsQCImsOrderEnquiryForm" method="POST" commandName="DsQCImsOrderEnquiryUI">
		
		<form:hidden path="reset"/>
					
					<div class="orderSearch">
						<h2 class="table_title" style="font-size: medium; margin: 0">Springboard Order Enquriy</h2>
						...
						<div class="overflowDiv">
							...
							<table style="margin-left: 5px; margin-top: 5px;">
								<tr>
									...
									<td style="width:110px;padding-left:100px">
										<b>End Date:</b>
									</td>
									<td style="width: 90px;">
								...
								<tr>
									...
									<td style="width: 90px;">
										<form:input id="orderId" path="orderId" />
									</td>
									<td style="width:110px;padding-left:100px">
										<b>Team :</b>
									</td>
								...
								<tr>
									...
									<td style="width: 90px;">
										<form:input id="serNum" path="serNum" />
									</td>
									<td style="width:110px;padding-left:100px">
										<b>Salesman Code:</b>
									</td>
					...
					<tr>
						<c:choose>
							<c:when test="${ imsOrderList.orderType eq 'NTV-A' || imsOrderList.orderType eq 'NTVAO' || imsOrderList.orderType eq 'NTVUS' || imsOrderList.orderType eq 'NTVRE' }">
								<td><a href="###" onclick="openDialog4SumNTV('${imsOrderList.orderId}')" title="Order Enquiry for IMS">${imsOrderList.orderId}</a><c:if test='${empty imsOrderList.orderId}'>&nbsp;</c:if></td>
							</c:when>
							<c:otherwise>
								<c:choose>
						...
						<td height="30">
								...
								<c:choose>
								<c:when test="${fn:containsIgnoreCase(imsOrderList.orderType, 'NTV')}">
								<c:choose>
								...
								<c:when test="${imsOrderList.orderStatus=='11'}">
								...
								<a class="nextbutton"href="${ntvDomain}ntvorderdetail.html?_al=new&orderId=${imsOrderList.orderId}&status=${imsOrderList.orderStatus}"
											title="Recall Order ${imsOrderList.orderId}">Recall&nbsp;</a>								
								</c:when><c:when test="${imsOrderList.orderStatus=='02' || imsOrderList.orderStatus=='04'}">
								...
								<c:choose><c:when test="${imsOrderList.orderStatus=='02' || imsOrderList.orderStatus=='04'}">
															<a class="nextbutton"
									href="#"
