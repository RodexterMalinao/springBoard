...
//			handleWaiveReason();
		...
		$('a#submit').click(function(event) {
			...
			if($("input[name=distributionMode]:checked").val() == "E" && $("#distributeEmail").val() == ""){
				...
				var result = confirm(noEmailAlert);
				if (!result) {
					return;
				}
			...
			}else if($("input[name=distributionMode]:checked").val() == "P" && !isBrCust){
				
				var result = confirm(paperModeAlert);
				if (!result) {
