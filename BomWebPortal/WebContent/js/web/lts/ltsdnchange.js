var ltsdnchange = function() {
			...
			$.ajax({
				...
				success : function(result) {
					if (result.status == 'true') {
						...
						$('form[name="ltsDnChangeForm"]').submit();
					} else {					
	...
	function chkPipbDn() {
		var sbuid = $('input[name="sbuid"]').val();
		var srvNum = $("#pipbSrvNum").val();
		var acctCd = $("#pipbAccountCd").val();
		var boc = $("#pipbBoc").val();
		var projectCd = $("#pipbProjectCd").val();
		var url = "ltsacqreserveddnlkup.html?sbuid=" + sbuid + "&srvNum=" + srvNum + "&acctCd=" 
		          + acctCd + "&boc=" + boc + "&projectCd=" + projectCd;		
		if (srvNum=='') {
			alert('Please input the PIPB DN');
		} else if (srvNum.length<8) {
			alert('Invalid service number');	
		} else {	
			$.ajax({
