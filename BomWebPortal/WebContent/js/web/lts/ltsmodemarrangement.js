var ltsmodemarrangement = function() {
	...
	function performModemLkup(fsa, fsaType, newService) {
		...
		var url = 'ltsmodemlkup.html?sbuid=' + sbuid + '&existingService=' + existingService + '&newService=' + newService;
	...
	function showLostModemDiv(){
		var profileFsa = $('#profileFsa').val();
		var modemType = $('input[name="modemType"]:checked').val(); 
		switch (modemType)  {
			case "SHARE_EX_FSA" :
				var selectedFsa = $('.existingFsaCheckbox:checked').attr('id');
				if(profileFsa != '' && selectedFsa == profileFsa){
				...
				break;
			case "SHARE_OTHER_FSA" :
				var selectedFsa = $('.otherFsaCheckbox:checked').attr('id');
				if(profileFsa != '' && selectedFsa == profileFsa){
					if($("[name=rentalRouterInd]:checked").val() == "SHARE_RENTAL_ROUTER"){
						$("#lostModemCheckbox").attr('disabled', true);
					}else{
	...
	function promptShareFsaAlert() {
		...
		if ($('input[name="modemType"]:checked').val() == "SHARE_EX_FSA") {
			...
			var preSelectFsa = $('input[name="preSelectFsa"]').val();
			if ( preSelectFsa != "" && preSelectFsa != null 
					&& selectedFsa != preSelectFsa && 
						$('input[id="existingFsa.existingService' + selectedFsa + '"]').val() == "WG") {
				return confirm("1AMS: Are you confirm to ride on WG upgrade to eye instead of share with PCD / TV modem?");
			}
		}
		return true;
	}
	...
	function promptAbnormalAddrCoverageMsg(){
		if ($('input[name="modemType"]:checked').val() == 'STANDALONE') {
			if($('#abnormalAddrCoverageInd').val() == 'PON_SHORTAGE'){
				var confirmed = confirm(ponShortageMsg);
				return confirmed;
			}
			if($('#abnormalAddrCoverageInd').val() == 'PON_BLOCKAGE'){
				var confirmed = confirm(ponBlockageMsg);
				return confirmed;
			}
		}
		return true;
	...
	function actionPerform() {
		...
		$('#continueBtn').click(function (event) {
			...
			if (!promptShareFsaAlert()) {
				return;
			}
			if(!promptAbnormalAddrCoverageMsg()){
				return;
			}
			submitForm("SUBMIT");
		});
