var ltsothervoiceservice = function() {
	...
	function handleDuplexSrv() {
		if($('input[name="duplexProfile"]').val() == "true") {
			
			if ( $('input[name="isRenewalOrder"]').val() == "false" || ($('input[name="isRenewalOrder"]').val() == "true" 
					&& $('input[name="removeRenewalDuplex"]').val() == "true" )) 
			
			if ($('input[name="duplexXSrvType"]:checked').val() == "CANCEL" 
				|| $('input[name="duplexYSrvType"]:checked').val() == "CANCEL") {
				$('#duplexSrvDiv').show();
				$(".new2ndDel").attr('disabled', '');
				$('tr.duplex2ndDelAutoInVasList').hide();
				$('tr.new2ndDelAutoChangeTpList').show();
				$('#secondDelRedeemPremiumDiv').show();
			}
			else {
				$('select[name="new2ndDelSrvStatus"]').val('20');
				$('input[name="new2ndDelDn"]').val('');
				$(".new2ndDel").attr('disabled', 'disabled');
				$('tr.duplex2ndDelAutoInVasList').show();
				$('tr.new2ndDelAutoChangeTpList').hide();
				$('#secondDelRedeemPremiumDiv').hide();
			}
		}
	}
	...
	function actionPerform() {
		...
		$('.duplexSrv').change(function(event) {
			handleDuplexSrv();
		});
