...
public class CcImsOrderEnquiryController extends SimpleFormController {
...
//		logger.info("terminationViewUrl:"+terminationViewUrl);
		...
		CcLtsImsOrderEnquiryUI enquiry = (CcLtsImsOrderEnquiryUI)request.getSession().getAttribute("ccimsorderenquiry");
	...
	public ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws ServletException {		
		logger.info("CcImsOrderEnquiry onSubmit is called");
...
//		if(!"Y".equalsIgnoreCase(enquiry.getReset())&&!(enquiry.getLob().equalsIgnoreCase("LTS"))){
		if(!"Y".equalsIgnoreCase(enquiry.getReset())){
			List<ImsAlertMsgDTO> imsOrderList =imsOrderAmendservice.getImsOrderEnquiryListInfo(enquiry, bomSalesUserDTO);
		...
		request.getSession().setAttribute("ccimsorderenquiry", enquiry);
		return new ModelAndView(new RedirectView("ccimsorderenquiry.html"));
	}
