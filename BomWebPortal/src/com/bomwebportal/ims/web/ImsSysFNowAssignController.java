...
public class ImsSysFNowAssignController  implements Controller{
	...
	public ModelAndView handleRequest(HttpServletRequest arg0,
		...
		if(!isProcessing){
			...
			try{
				...
				List<OrderImsSystemFindingDTO> imsDTO = imsOrderDetailService.sysFcheckingNowTV();
				...
				for (int i=0;i<imsDTO.size();i++){
						...
						if (imsDTO.get(i).getSysF().length()==0){
							...
							//	}                                                                                                                 
								...
								if ("PASS".equals(imsDTO.get(i).getIdDocType())) {                                               
									searchKeyDto.setIdDocType("PASS");		                                                                          
								} else if ("BS".equals(imsDTO.get(i).getIdDocType())) {                                              
									searchKeyDto.setIdDocType("BS");                                                                                
								} else if ("CERT".equals(imsDTO.get(i).getIdDocType())) {                                  
								...
								logger.debug("Offer searchKeyDto: "+ searchKeyDto);                                            
								...
								if (serviceLineDTOList.size() > 0){                                                                                 
									...
									for (int j=0;j<serviceLineDTOList.size();j++) { 
										...
										if (serviceLineDTOList.get(j).getSystemId().equalsIgnoreCase(ImsConstants.IMS_ORDER_LOB)){
											//check = imsOrderDetailService.checkNowTVexistingServcie(serviceLineDTOList.get(j).getServiceNum());
											//if (check){imsDTO.get(i).setSysF("1D1I"); break;}
											Ims1AMSInfoWithoutPendingDTO ims1AMSInfoWithoutPendingDTO = ims1AMSEnquiryService.getIms1AMSInfoWithoutPending(serviceLineDTOList.get(j).getServiceNum());
											logger.debug("IS standalone nowTV: " + ims1AMSInfoWithoutPendingDTO.getIsStandAloneTV());
											if (ims1AMSInfoWithoutPendingDTO.getIsStandAloneTV().equalsIgnoreCase("Y")){
												imsDTO.get(i).setSysF("1D1I");
