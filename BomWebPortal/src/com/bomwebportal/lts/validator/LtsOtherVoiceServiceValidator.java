...
public class LtsOtherVoiceServiceValidator implements Validator {
	...
	public boolean supports(Class clazz) {
		return LtsOtherVoiceServiceFormDTO.class.isAssignableFrom(clazz);
	}
	...
	public void validate(Object command, Errors errors) {
		...
		case SUBMIT:
			...
			//Validate DN change and Duplex Handing option
			if(form.isDuplexProfile() && form.isDnChange()){
				if (DuplexSrvType.CANCEL == form.getDuplexXSrvType() && form.getDnChangeNumX() != null) {
					errors.rejectValue("duplexDnChange", "lts.duplexDnChange.invalid");
				}
				if (DuplexSrvType.CANCEL == form.getDuplexYSrvType() && form.getDnChangeNumY() != null) {
					errors.rejectValue("duplexDnChange", "lts.duplexDnChange.invalid");
				}
			...
			if (!ltsOfferService.isItemSelected(form.getSecondDelHotVasItemList(), LtsConstant.ITEM_TYPE_VAS_2DEL_HOT)) {
...
//			else if ("HKID".equals(form.getSecondDelDocType())
//					&& !form.getSecondDelDummyDoc().booleanValue()
//					&& (!Utility.validateHKID(form.getSecondDelDocNum())
//						|| !Utility.validateHKIDcheckDigit(form.getSecondDelDocNum()))
//						) {
//				errors.rejectValue("secondDelDocNum", "lts.docNum.invalid");
//			}					
			break;
