...
public class CcsCreateBomOrderHistory {
	...
	public void processCcsRequest(String orderId, String seqNo) {

		logger.info("Create CCS BOM History order for orderId: " + orderId +", Seq No: "+seqNo);
		try {
		    ...
		    orderHistoryService.updateBomWebSim(newMobileSimInfo,seqNo);
			
			
			/**************** Get the data from database ***************/
			OrderDTO orderDto = orderHistoryService.getOrder(orderId,seqNo);
			CustomerProfileDTO customerInfoDto = orderHistoryService.getCustomerProfile(orderId,seqNo);
			MnpDTO mnpDto = orderHistoryService.getMnp(orderId,seqNo);
			PaymentDTO paymentDto = orderHistoryService.getPayment(orderId,seqNo);
			MobileSimInfoDTO mobileSimInfo = orderHistoryService.getSim(orderId,seqNo);
			SubscriberDTO subscriberDto = orderHistoryService.getBomWebSub(orderId,seqNo);
			AccountDTO accountDto = orderHistoryService.getAccount(orderId,seqNo);
			List<ComponentDTO> componentList = orderHistoryService.getPassToBomComponentList(orderId,seqNo);//getComponentList
			//For getting Golden Number & 1C2N
		    ArrayList<MobCcsMrtBaseDTO> mobCcsMrtDtoList = orderHistoryService.getMobCcsMrtDTO(orderId,seqNo);
		    //String multiIMEIs ="";
		    String multiIMEIs = orderHistoryService.getMultiIMEI(orderId,seqNo);
			...
			List<com.pccw.bom.mob.schemas.ProductDTO> productList = orderHistoryService
					.getBomProductList(orderDto.getOrderId(),seqNo);
			...
			bulkNewActClient.bulkNewActCcsHistClient(orderDto.getOrderId(),
					bomCustomerDto, bomAccountDto, bomCreateOrdReqDTO, orderDto.getLastUpdateBy(),seqNo);
